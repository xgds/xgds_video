...
def startFlightRecording(flightName):
    ...
    # TODO: This is not true - look up timecode from Wowza playlist
    startTime = timezone.now() - timedelta(seconds=settings.XGDS_VIDEO_SEGMENT_START_TIME_OFFSET_SECONDS)
    try:
...
def makeNewSegment(source, recordingDir, recordingUrl, startTime, episode):
    ...
    try:
        maxSegmentNumber = SEGMENT_MODEL.get().objects.filter(episode=episode, source=source).aggregate(
            Max('segNumber'))
...
def startRecording(source, recordingDir, recordingUrl, startTime, episode):
    ...
    if settings.XGDS_VIDEO_RECORDING_METHOD == 'VLC':
        ...
        recorderCommand = ("%s %s --sout='#duplicate{dst=std{access=livehttp{seglen=6,splitanywhere=false,delsegs=false,numsegs=0,index=prog_index.m3u8,index-url=prog_index-#####.ts},mux=ts,dst=prog_index-#####.ts}}'" % \
                          (settings.XGDS_VIDEO_VLC_PATH, segmentInfo['videoFeed'].url))
        ...
        recorderCommand = ('%s --sourceUrl=%s --outputDir=%s --recorderId=%s --episodePK=%d --sourcePK=%d' % (
                           scriptPath, segmentInfo['videoFeed'].url, segmentInfo['recordedVideoDir'], assetName,
                           episode.pk, source.pk))
